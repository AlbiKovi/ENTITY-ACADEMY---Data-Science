...
# Grouping only works with summarize function
Pets14 <- group_by(Pets, Animal)
Pets14

# Summarize data of entire column or by groups
MeanWeight1 <- summarize(Pets, mean = mean(Weight))
View(MeanWeight1)
#Average of weight column

# Average weight by animal type
...
CountPets <- Pets %>% group_by(Animal) %>% summarize(Count = n())
CountPets 

# Group by multiple variables
PetGrowth <- Pets %>% group_by(Animal, Age, Weight) %>% summarize(Count = n())
PetGrowth 

#Shows only ages of pets
summarize(Pets , Age)